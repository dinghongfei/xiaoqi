---
name: convert-wechat
description: 把 processed.md 换成公众号预览页：左侧文章预览（手机/电脑），右侧可调字体/字号/主题色，点「复制正文」贴进公众号。
---
...
- 预览页左侧是文章（可切手机 / 电脑宽度），右侧可调字体、字号、主题色。有封面时文章最上方先展示封面图（宽与正文区相同，高按 2.35:1，鼠标悬停可看完整原图）、提示文字和横线。预览用 CSS 变量；点「复制正文」时在浏览器里把当前计算样式写成 inline，并把图片嵌进剪贴板。图片按原图像素大小粘贴，不跟手机/电脑预览宽度走，由你在公众号编辑器里决定是否裁剪。「复制封面图」默认勾选，会带上封面、提示和横线；取消勾选则只复制正文。不要单独复制图片（`127.0.0.1` 公众号后台拉不到）。
- 高亮块 / 视频卡片 / 代码标题栏用 `section`（标题文字再包一层 `span`）。公众号编辑器会丢掉 `aside`、`div` 上的背景色和标题颜色；复制时还会把残留的 `aside`/`div` 改成 `section`。
- 没有「发布」按钮，也不接主题市场。
- 成功时打印 `公众号预览=...`
