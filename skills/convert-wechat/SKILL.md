---
name: convert-wechat
description: 把 processed.md 换成公众号预览页：左侧文章预览（手机/电脑），右侧可调字体/字号/主题色，点「正文复制」贴进公众号。
---
...
- 预览页左侧是文章（可切手机 / 电脑宽度），右侧可调字体、字号、主题色。预览用 CSS 变量；点「正文复制」时在浏览器里把当前计算样式写成 inline，并把图片嵌进剪贴板。不要单独复制图片（`127.0.0.1` 公众号后台拉不到）。
- 没有「发布」按钮，也不接主题市场。
- 成功时打印 `公众号预览=...`
