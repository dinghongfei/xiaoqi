...
`apply` 成功：中文说明已写入本地路径，并打印写回云文档要用的 lark-cli 命令（含缺封面时的 `media-insert`，以及插图后重新 fetch / `download-feishu-doc`）。产物包括 `processed.md`（属性表，不含封面图文件，不改 `raw.md`）和 `data/jobs/<token>/enrich.xml`。XML 里没有生图提示词。封面以飞书为准，插图后必须重新下载。
