"""Restyle Feishu markdown into a standalone WeChat preview HTML page."""
...
from feishu_text import extract_summary, extract_title, prepare_feishu_markdown
from highlight import highlight_code, parse_fence_info
from themes import (
    ARTICLE_PREVIEW_CSS,
    COLORS,
    FONTS,
...
from xml_styles import StyleOverlayError, overlay_xml_styles
...
def _media_src(src: str) -> str:
    """Keep Feishu http(s) URLs and relative paths as-is."""
    return (src or "").strip()


_FIRST_MD_IMAGE = re.compile(r"!\[([^\]]*)\]\(([^)\s]+)\)")
_FIRST_HTML_IMG = re.compile(
    r"<img\b[^>]*\bsrc\s*=\s*[\"']([^\"']+)[\"']",
    re.IGNORECASE,
)
_FIRST_FEISHU_IMAGE = re.compile(
    r"<image\b[^>]*\b(?:src|href)\s*=\s*[\"']([^\"']+)[\"']",
    re.IGNORECASE,
)
_FIRST_FIGURE_SC = re.compile(
    r"\{\{<\s*figure\s+[^>]*\bsrc=\"([^\"]+)\"",
    re.IGNORECASE,
)


def pick_first_image_src(markdown_text: str) -> str:
    """Return the first image URL in the article body, or empty string."""
    text = markdown_text or ""
    candidates: list[tuple[int, str]] = []
    for pattern, group in (
        (_FIRST_MD_IMAGE, 2),
        (_FIRST_HTML_IMG, 1),
        (_FIRST_FEISHU_IMAGE, 1),
        (_FIRST_FIGURE_SC, 1),
    ):
        match = pattern.search(text)
        if match:
            src = _media_src(match.group(group))
            if src:
                candidates.append((match.start(), src))
    if not candidates:
        return ""
...
COVER_HINT = (
    "此为封面图（取自正文第一张图）。复制到公众号编辑器后，请先设置封面，再手动删除本提示和横线。"
    ...
    *,
    ...
    def replace_video(match: re.Match[str]) -> str:
        ...
        return _wx_card(
            "wechat-video-card",
            "🎬 视频",
            f"<p>视频不在公众号内嵌播放：{html.escape(label)}</p>",
        ).strip()
    ...
    def replace_md_image(match: re.Match[str]) -> str:
        alt, src = match.group(1), _media_src(match.group(2))
        img = (
...
def _style_theme_cards() -> str:
    bits: list[str] = []
    for key, label, desc, _accent, _font in STYLE_THEMES:
        bits.append(
            f'<button type="button" class="wx-style-card" data-opt="style" '
            f'data-value="{html.escape(key)}" aria-pressed="false">'
            f'<span class="wx-style-card__name">{html.escape(label)}</span>'
            f'<span class="wx-style-card__desc">{html.escape(desc)}</span>'
            f"</button>"
        )
    return f'<div class="wx-style-cards">{"".join(bits)}</div>'


def _panel_html(*, title: str = "", summary: str = "") -> str:
...
def _chrome_css() -> str:
    ...
    * { box-sizing: border-box; }
    ...
    .wx-style-card[aria-pressed="true"] {
      border-color: #111827;
...
def _preview_script() -> str:
    ...
    (function () {
      ...
      const LABEL = "复制正文";
      const storageKey = "wx-preview-style-v2:" + (document.body.getAttribute("data-slug") || "default");
      const STYLE_PROPS = [
      ...
      function apply() {
        const styleId = state.style || "classic";
        host.dataset.style = styleId;
      ...
      async function inlineImages(root) {
        ...
        for (const img of imgs) {
          ...
          if (!res.ok) throw new Error("图片打不开（飞书链接可能已过期或需登录）");
      ...
      document.addEventListener("click", (ev) => {
        ...
        if (opt === "size") state.size = Number(value);
          ...
          state.style = value;
          const pack = (data.styles || {})[value];
          if (pack) {
            if (pack.accent) state.accent = pack.accent;
            if (pack.font) state.font = pack.font;
          }
...
def build_preview_page(
    ...
    *,
    ...
    panel = _panel_html(title=title, summary=summary)
...
<head>
  ...
  <meta name="doubao-hide-publish" content="1">
  <meta name="doubao:hide-actions" content="publish,download">
...
<body data-slug="{safe_slug}" data-title="{safe_title}">
  <div class="wx-app">
    <div class="wx-body">
      <div class="wx-stage">
        ...
        <main class="wx-preview">
          <div id="wx-device" data-device="phone">
            <div id="article-html" data-style="classic">{cover_html}{article_html}</div>
          </div>
        </main>
      </div>
      {panel}
...
(function () {{
  try {{
    ...
    for (var i = 0; i < nodes.length; i++) {{
      ...
      if (/发布/.test(text) || /download|下载/i.test(text) || /download/i.test(el.className || "")) {{
...
def convert_wechat(
    *,
    ...
    """Write a standalone WeChat preview HTML."""
    from media_local import MediaMissingError, MediaProcessor, embed_local_media_in_html

    prepared = prepare_feishu_markdown(markdown_text)
    article_title = extract_title(markdown_text, fallback=token) or token or "公众号预览"

    work_dir = out_path.resolve().parent
    # Media files live next to raw.md / raw.xml / preview html
    media_root = Path(media_dir) if media_dir else work_dir
    processor = MediaProcessor(work_dir=media_root)
    try:
        prepared = processor.process_body(prepared, xml_text=xml_text)
    except MediaMissingError as exc:
        return WeChatResult(
            status="error",
            message=(
                f"❌ 无法下载媒体：{exc.token}\n"
                f"可手动执行：lark-cli docs +media-download --token '{exc.token}' "
                f"--output '{media_root / exc.token}'"
            ),
            title=article_title,
        )

    # Persist rewritten body (local filenames) for debugging
    try:
        (work_dir / "body.md").write_text(prepared, encoding="utf-8")
    except OSError:
        pass

    cover_src = pick_first_image_src(prepared)
    from feishu_text import clamp_summary

    if summary is not None and str(summary).strip():
        summary_text = clamp_summary(str(summary), max_chars=100)
    else:
        summary_text = extract_summary(prepared, max_chars=100)
    try:
        article = restyle_markdown(
            prepared,
            xml_text=xml_text,
    ...
    page = build_preview_page(
        article,
        title=article_title,
        slug=token or out_dir.name,
        summary=summary_text,
        cover_image=cover_src,
    )
    page = embed_local_media_in_html(page, out_dir)
    out_path.write_text(page, encoding="utf-8")
    note = f"公众号预览已生成 {out_path}"
    if cover_src:
        note += "（封面取自正文第一张图）"
    else:
        note += "（正文无图，已跳过封面）"
