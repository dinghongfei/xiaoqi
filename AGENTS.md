...
uv run python skills/enrich-doc/scripts/run.py apply --token '…' --slug … --lang zh --title '…' --date YYYY-MM-DD --author '小七' --categories '…' --summary '…'
...
flowchart TB
  ...
  subgraph enrichFlow["补全（不转换）"]
    ...
    wr --> cover[缺封面则 media-insert]
    cover --> dl2[插图后重新 download-feishu-doc]
  end
